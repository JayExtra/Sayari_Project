...
import com.dev.james.sayariproject.utilities.FIVE_MIN_NOTIFICATION_CHANNEL
import com.dev.james.sayariproject.utilities.SAYARI_MAIN_NOTIFICATION_CHANNEL
import java.util.*

class FiveMinuteLaunchAlertReceiver : BroadcastReceiver() {
    ...
    private fun showNotification(context: Context?,name: String?, slug: String?) {
        if (context != null) {
            val notBuilder = NotificationCompat.Builder(context, FIVE_MIN_NOTIFICATION_CHANNEL)
                .setSmallIcon(R.drawable.ic_launcher_background)
                .setContentText("$slug launch window will open in 5 minutes")
                .setContentTitle(name)
                .build()

            notificationManager.notify(
                FIVE_MIN_LAUNCH_NOTIFICATION_ID , notBuilder
            )
